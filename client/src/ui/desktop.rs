...
use crate::app::{
    ConnectButton, ConnectPage, HostField, InfoLabel, InfoMessage, PlayerName, PlayerNameField,
    PortField, ProgramOptions, State,
};
use crate::ui::{HOVERED_BUTTON, NORMAL_BUTTON};
...
pub fn handle_connect_page(
    ...
    mut program_options: ResMut<ProgramOptions>,
    ...
    for (entity, _, interaction, mut color, _, _) in &mut connect_button {
        match *interaction {
            Interaction::Pressed => {
                ...
                {
                    info_label.write(InfoMessage("Invalid host".to_string()));
                    return;
                }
                if maybe_port_field_int.is_err() {
                    info_label.write(InfoMessage("Invalid port".to_string()));
                    return;
                }
                program_options.hostname = host_field_string;
                program_options.port = maybe_port_field_int.unwrap();
